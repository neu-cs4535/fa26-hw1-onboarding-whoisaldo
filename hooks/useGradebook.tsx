...
export function useGradebookColumnGroups() {
  const controller = useGradebookController().gradebook_column_groups;
  const [groups, setGroups] = useState(controller.rows);
  useEffect(() => {
    const { data, unsubscribe } = controller.list(setGroups);
    setGroups(data);
    return unsubscribe;
  }, [controller]);
  return groups;
